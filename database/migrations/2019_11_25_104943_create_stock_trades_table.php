...
class CreateStockTradesTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('stock_trades', function (Blueprint $table) {
            ...
            $table->decimal('before_amount')->comment('이전금액');
            $table->decimal('calc_amount')->comment('가감금액');
            $table->decimal('fee_amount')->comment('수수료');
            $table->decimal('now_amount')->comment('현재금액');
